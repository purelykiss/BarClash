...
public partial class WarriorSweep1Attack : Area2D, IAttackBox
{
    ...
    [Export] int _damage = 100;
    [Export] float _atkCooltime = 100;
    float _curAtkCooltime;
    [Export] Vector2 _pushPower = new Vector2(100,150);
    /// <summary>
    /// 0: 없음 1: 약경직 2: 강경직 3: 다운
    /// </summary>
    [Export] int _stagger = 3;
    ...
    [Export] bool _hasFinishMotion = true;
    bool _hasEffect;
    bool flagEffect;
    ...
    void CheckAttack()
    {
        foreach (var item in _UnitAndAttacktedInZone)
        {
            ...
            if (flagAttack)
            {
                ...
                _warriorMove.IsTryingSweepCombo = true;
            }
        }
    }
