...
public partial class melee1 : Node, IState
{
    ...
    public void Move()
    {
        if (_isTryingActivateAttack)
        {
            if (!flagActivateAttack)
            {
                ...
                _baseMove.SndList.ActivateSound("punchLow");
            }
        }
        else
        {
        ...
        if (_playerMove.IsDead
            || _curDuration <= 0
            || (_curDuration <= 0.6f && (Input.IsActionPressed("player_left")|| Input.IsActionPressed("player_right")))
            || !_playerMove.IsGrounded)
            _curCancelState = IState.Cancelable.Always;
        else if(_curDuration <= 0.7f)
    ...
    public void OnFinish()
    {
        ...
        _playerMove.AtkList.DisableAttackBox("melee1");
    }
}
