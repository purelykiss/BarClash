...
public partial class straight : Node, IState
{
    [Export] PlayerMove _playerMove;
    [Export] string _id = "straight";
    ...
    public bool Condition()
    {
        ...
        if (_playerMove.IsTryingStraight)
        {
            if (_playerMove.HasStraight)
                return true;
        }
    ...
    public void Animation()
    {
        ...
        {
            flagAnimate = true;
            _baseMove.SetAnimation("straight", false);
        }
    }
    ...
    public void Move()
    {
        if (_isTryingActivateAttack)
        {
            if (!flagActivateAttack)
            {
                ...
                _baseMove.SndList.ActivateSound("punchHigh");
            }
        }
        else
        {
        ...
        if (flagFirstCycle)
        {
            ...
            _playerMove.IsTryingStraight = false;
            _playerMove.HasStraight = false;
            _curDuration = _duration;
            ...
            switch (_baseMove.CurDirection)
            {
                case DirectionH.LEFT:
                    _baseMove.CurVelocity = new Vector2(-_playerMove.CurRunSpeed * 5f, _baseMove.PreVelocity.Y);
                    break;
        ...
        {
            float xSpeed = Mathf.Lerp(_baseMove.PreVelocity.X, 0, 0.2f);
    ...
    public void OnFinish()
    {
        ...
        _playerMove.AtkList.DisableAttackBox("straight");
        GD.Print("Call Disable " + ID);     //디버그
    }
}
