...
public partial class WarriorSweep1 : Node, IState, IEnemyPattern
{
    [Export] WarriorMove _warriorMove;
    [Export] string _id = "sweep1";
    ...
    public bool Condition()
    {
        ...
        if (_baseMove.IsGrounded)
        {
            if (_warriorMove.IsTryingAttack)
            {
                ...
                {
                    if (Mathf.Abs(_warriorMove.TargetUnit.GlobalPosition.X - _baseMove.GlobalPosition.X) < 100)
                        return true;
                }
            }
        }
    ...
    public void Animation()
    {
        ...
        {
            flagAnimate = true;
            _baseMove.SetAnimation("sweep1", false);
        }
    }
    ...
    public void Move()
    {
        if (_isTryingActivateAttack)
        {
            if (!flagActivateAttack)
            {
                ...
                _baseMove.SndList.ActivateSound("swordHigh");
            }
        }
        else
        {
        ...
        if (flagFirstCycle)
        {
            ...
            _curDuration = _duration;
            _baseMove.SndList.ActivateSound("wind");
        }
        else
        {
    ...
    public void OnFinish()
    {
        ...
        _warriorMove.AtkList.DisableAttackBox("sweep1");
    }
}
