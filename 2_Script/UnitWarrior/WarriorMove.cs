...
public partial class WarriorMove : CharacterBody2D, IUnitMove
{
    ...
    bool _flagEnemyDetected;
    IUnitMove _targetUnit;
    IUnitMove _lastTargetUnit;
    ...
    public bool FlagEnemyDetected => _flagEnemyDetected;

    public IUnitMove TargetUnit
    {
        get
        {
            if (_targetUnit != null)
                return _targetUnit;
            else if (_lastTargetUnit != null)
    ...
    public IUnitMove LastTargetUnit => _lastTargetUnit;
    ...
    public override void _Ready()
    {
        ...
        _curPhase = 1;
        flagPhase = 1;
        flagCustomDodge = false;
        flagForceSlash = false;
        _isGrounded = true;
        _isDown = false;
        _isRunning = false;
    ...
    private void CheckAiState()
    {
        ...
        switch(_curAiState)
        {
            case IUnitMove.AIState.WAIT:
                ...
                if( _curAiWaitTime <= 0 || flagForceSlash)  //페이즈 변경시 flagForceSlash로 바로 CHASE로 변경
                {
                    _curAiWaitTime = _aiWaitTime;
            ...
            case IUnitMove.AIState.CHASE:
                ...
                if(flagChaseFirstCycle)
                {
                    ...
                    if(flagForceSlash)     //페이즈 변경시 강제 패턴 Slash
                    {
                        flagForceSlash = false;
                        _nextPatternID = "slash1";
                    }
                    _isTryingChase = true;
